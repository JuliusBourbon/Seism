...
const ConfirmModal = ({ isOpen, title, message, onConfirm, onCancel, isDestructive }) => {
    ...
    return (
        <div className="fixed inset-0 rounded-2xl z-9999 flex items-center justify-center bg-black/30 backdrop-blur-[1px] p-4" onClick={(e) => e.stopPropagation()}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[320px] p-6">
                <h3 className={`text-lg font-bold mb-2 ${isDestructive ? 'text-red-600' : 'text-blue-600'}`}>{title}</h3>
                <p className="text-gray-600 text-sm mb-6">{message}</p>
                <div className="flex gap-3 justify-end">
                    <button onClick={onCancel} className="px-4 py-2 rounded-lg text-sm font-bold text-gray-500 hover:bg-gray-100">Batal</button>
                    <button onClick={onConfirm} className={`px-4 py-2 rounded-lg text-sm font-bold text-white shadow-lg ${isDestructive ? 'bg-red-500 hover:bg-red-600' : 'bg-blue-600 hover:bg-blue-700'}`}>Ya, Lanjutkan</button>
                </div>
            </div>
        </div>
    );
...
export default function ReportPopup({ report, currentUser }) {
    ...
    const requestDelete = () => {
        ...
        setConfirmDialog({
            isOpen: true,
            title: "Konfirmasi",
            message: "Laporan yang dihapus tidak dapat dikembalikan lagi",
    ...
    const requestResolve = () => {
        ...
        setConfirmDialog({
            isOpen: true,
            title: "Konfirmasi",
            message: "Ubah status laporan ini menjadi 'Resolved'?",
    ...
    const executeResolve = async () => {
        try {
            ...
            if (response.ok) {
                showNotification(
                    "Sukses",
                    "Berhasil mengubah status laporan",
                    "success",
                    "Tutup"
                );
                window.location.reload();
    ...
    const handleVote = async (type) => {
        if (!currentUser) {
            showNotification(
                "Perlu Login",
                "Login terlebih dahulu untuk memberikan vote",
                "error",
                "Tutup"
            );
            return;
